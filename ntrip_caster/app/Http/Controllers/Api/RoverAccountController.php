...
class RoverAccountController extends Controller
{
    public function index(
        ...
        $accounts = RoverAccount::query()
            ...
            ->when(
                $status === 'disabled',
                fn (Builder $query): Builder => $query->where('enabled', false)
            )
            ->when(
                $status === 'expired',
    ...
    public function update(
        ...
        DB::transaction(
            function () use (
                ...
                if ($updates !== []) {
                    $roverAccount->update($updates);
                }

                /*
                 * Không để quyền riêng trên Mountpoint vượt quá
                 * giới hạn mới của tài khoản.
                 */
                if (
                    array_key_exists(
                        'max_connections',
                    ...
                    DB::table(
                        ...
                        ->update([
                            'max_connections' => $roverAccount->max_connections,
                            'updated_at' => now(),
                        ]);
                }
            }
    ...
    public function destroy(
        ...
        if (
            ...
            return response()->json([
                'message' => implode(' ', [
                    'Không thể xóa Rover Account',
                    'đang có kết nối hoạt động.',
                ]),
            ], 409);
        }

        DB::transaction(
            function () use ($roverAccount): void {
                $roverAccount->update([
                    'enabled' => false,
                ]);
